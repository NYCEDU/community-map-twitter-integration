//v1.1 (please update this version number anytime the code below is deployed, i.e. cut and pasted from GitHub into this spreadsheet's Google script).
...
function getTwitterDescription(username) { //argument 'username' is an array of Twitter usernames supplied by column D of the spreadsheet
      ...
      if (service.hasAccess()) { //if we're here, that means there was nothing in the cache, and we should retrieve the information from the Twitter API
        ...
        var cacheTimeLimit = Math.floor((Math.random() * 21600) + 7200); //assign a random expiration to each description stored in the cache, so that everything doesn't expire at the same time.
